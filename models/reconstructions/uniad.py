...
class UniAD(nn.Module):
    ...
    def forward(self, input):
        ...
        # For Classification
        # print(input.keys())
        input.update(self.vitclassifiy(input))
        # print(input.keys())
...
class Transformer(nn.Module):
    ...
    def forward(self, src, pos_embed, class_condition):
        ...
        for i in range(4):
            ...
            result = rearrange(result.permute(0, 2, 3, 1), 'b h w c -> b (h w) c')  # torch.Size([3, 196, 256])
            result = rearrange(result, 'b hw c -> hw b c')  # torch.Size([196, 3, 256])
            output_tensor.append(result)
